...
/// @title MeadowArt
/// @notice ERC-721 art pieces bought with a project token, each streaming
/// rewards in one assigned Robinhood Stock Token per second the piece is
/// held. Rewards are best-effort and treasury-funded: there is no reserve
/// commitment or IOU. If the assigned stock's balance runs dry, claims pay
/// whatever is available and the unpaid remainder is not tracked or owed.
contract MeadowArt is ERC721, Ownable2Step, ReentrancyGuard {
    ...
    constructor(
        ...
        uint64 rewardEnd_,
        address owner_
    ) ERC721("meadow art", "PIECE") Ownable(owner_) {
