...
/// @notice Deploys MeadowArtV2. Per-work economics (price, assigned stock
/// index, reward rate) come from the v2 catalog generator's output at
/// art/data/onchain2.json, same schema as onchain.json. Real addresses and
/// the reward deadline are operator-supplied at broadcast time via env vars
/// -- nothing here is hardcoded, and there is no private key in this file;
/// forge script picks the signer up from --private-key, --ledger, or an
/// unlocked --sender.
///
/// Required env vars:
...
contract DeployMeadowArtV2 is Script {
    ...
    function run() external {
        ...
        console.log("MeadowArtV2:", address(art));
    ...
    /// @dev Reads the works array written by the v2 catalog generator.
        ...
        returns (uint256[] memory prices, uint8[] memory stockIdx, uint256[] memory rates)
    {
        ...
        require(n > 0, "onchain2.json has no works: run the v2 catalog generator first");
        require(n < MAX_WORKS, "onchain2.json has an implausible number of works, check the file");
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            require(id == i, "onchain2.json works array is not ordered by ascending id");

            prices[i] = vm.parseJsonUint(json, string.concat(base, ".price_wei"));
            require(prices[i] > 0, "onchain2.json has a zero price_wei entry");

            uint256 idx = vm.parseJsonUint(json, string.concat(base, ".stock_idx"));
            require(idx < stockCount, "onchain2.json stock_idx is out of range for STOCKS");
